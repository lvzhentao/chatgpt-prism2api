...
function createStdinChallengeReader() {
  const lines = readline.createInterface({ input: process.stdin, crlfDelay: Infinity });
  let consumed = false;

  return async () => {
    if (consumed) throw new Error("stdin challenge 只能读取一次");
    consumed = true;
    for await (const line of lines) {
      if (!line.trim()) continue;
      lines.close();
      const message = parseJson(line, "stdin challenge 消息");
      if (!message || typeof message !== "object" || Array.isArray(message)) {
        throw new Error("stdin challenge 消息必须是 object");
      }
      if (message.type !== "challenge") {
        throw new Error("stdin challenge 消息 type 必须是 challenge");
      }
      return normalizeChallenge(message.challenge);
    }
    throw new Error("stdin 在 challenge 消息前结束");
  };
}
...
async function main(argv = process.argv.slice(2), writeOutput = true) {
  ...
  const challengeStdin = args["challenge-stdin"] === "1";
  const officialMode =
    args.official === "1" ||
    truthy(cfg("official")) ||
    process.env.SENTINEL_OFFICIAL === "1" ||
    (!challengeFile && !challengeStdin && !args["challenge-url"] && !cfg("challengeUrl", "challenge_url") && !process.env.SENTINEL_CHALLENGE_URL);
  const challengeUrl =
  ...
  if (!challengeFile && !challengeUrl && !challengeStdin) {
    throw new Error("请提供 --challenge-file、--challenge-stdin、--challenge-url 或 --official，用于把题目服务器 challenge 喂回 SDK。");
  }
  if (challengeStdin && (challengeFile || args["challenge-url"] || args.official === "1")) {
    throw new Error("--challenge-stdin 不能与其他 challenge 来源同时使用");
  }

  let cachedChallenge = null;
  const readStdinChallenge = challengeStdin ? createStdinChallengeReader() : null;
  const options = {
    flow,
    ...
    async handleIframeMessage(message) {
      ...
      if (typeof proof !== "string" || !proof) {
        throw new Error("SDK 没有生成非空 challenge proof");
      }
      if (challengeStdin) {
        if (cachedChallenge !== null) {
          throw new Error("SDK 在同一 stdin 交换中重复请求 challenge");
        }
        process.stdout.write(`${JSON.stringify({ type: "proof", proof })}\n`);
        cachedChallenge = await readStdinChallenge();
  ...
  if (challengeStdin) {
    ...
    process.stdout.write(`${JSON.stringify({ type: "token", token: tokenText })}\n`);
